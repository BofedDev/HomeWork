...
Додайте кнопку “Show descriptors”, яка рендерить JSON дескрипторів моделі в <pre> на сторінці.ворити 2-3 екземпляри (конкретних студентів) і показати використання цих методів.о шукати індекс найближчого порожнього місця в масиві та перезаписувати значення.я пояснень того як правильно розділити - БЕЗ ГЕНЕРАЦІЇ КОДУ